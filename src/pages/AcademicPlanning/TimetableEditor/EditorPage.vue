  ...
  <div class="editor-page">
          ...
          <div class="d-flex align-center ga-3">
            ...
            <div>
              ...
              <p class="text-body-2 text-medium-emphasis mb-0">
                Review, correct, lock, version, publish and export generated timetables for
                <strong>{{ appStore.selectedAcademicYear?.name || 'the selected academic session' }}</strong>.
              </p>
            </div>
          </div>
          <div class="d-flex flex-wrap ga-2">
            <v-btn variant="tonal" prepend-icon="mdi-refresh" :loading="loading" :disabled="!selectedAcademicYearId" @click="loadTimetables">Refresh</v-btn>
            <v-btn color="primary" prepend-icon="mdi-auto-fix" :disabled="!selectedAcademicYearId" @click="router.push({ name: 'timetable.generator' })">Generate</v-btn>
          </div>
        </div>
      </v-card-text>
    </v-card>

    <v-alert v-if="!selectedAcademicYearId" type="warning" variant="tonal" class="mb-5" rounded="lg">
      Select an academic session from the app bar before opening or editing timetables.
    </v-alert>
    ...
    <v-row>
      <v-col cols="12" lg="3">
        <v-card rounded="xl" elevation="0" border class="browser-card">
          ...
          <v-card-text>
            <v-text-field v-model="search" prepend-inner-icon="mdi-magnify" label="Search timetable" variant="outlined" density="comfortable" hide-details clearable class="mb-3" :disabled="!selectedAcademicYearId" />
            <v-select v-model="statusFilter" :items="statusOptions" label="Status" variant="outlined" density="comfortable" hide-details clearable class="mb-3" :disabled="!selectedAcademicYearId" />
          </v-card-text>
          <v-divider />
          ...
          <v-list v-else lines="three" class="timetable-list">
            ...
            <div v-if="!filteredTimetables.length" class="text-center text-medium-emphasis pa-8"><v-icon size="48">mdi-calendar-blank-outline</v-icon><div class="mt-3">{{ selectedAcademicYearId ? 'No timetables found for this session.' : 'Select a session first.' }}</div></div>
          </v-list>
        </v-card>
      </v-col>
            ...
            <v-card-text class="pa-5">
              <div class="d-flex flex-wrap align-center justify-space-between ga-3">
                <div>
                  ...
                  <div class="text-body-2 text-medium-emphasis mt-1">{{ classLabel(timetable) }} · {{ entries.length }} entries · {{ lockedCount }} locked · {{ appStore.selectedAcademicYear?.name }}</div>
                </div>
                <div class="d-flex flex-wrap ga-2">
    ...
    <v-dialog v-model="entryDialog" max-width="760" persistent>
      <v-card rounded="xl"><v-card-title class="d-flex justify-space-between pa-5"><div><div>{{ editingEntry ? 'Edit timetable entry' : 'Add timetable entry' }}</div><div class="text-caption text-medium-emphasis font-weight-regular">Conflicts are checked automatically before saving in {{ appStore.selectedAcademicYear?.name || 'the selected session' }}.</div></div><v-btn icon="mdi-close" variant="text" @click="entryDialog = false" /></v-card-title><v-divider />
        ...
        <v-divider /><v-card-actions class="pa-5"><v-btn v-if="editingEntry" color="error" variant="text" :loading="deleting" @click="removeEntry">Delete</v-btn><v-spacer /><v-btn variant="text" @click="entryDialog = false">Cancel</v-btn><v-btn color="primary" :loading="saving" :disabled="!selectedAcademicYearId" @click="saveEntry">Save Entry</v-btn></v-card-actions>
      </v-card>
    </v-dialog>

    <v-dialog v-model="versionDialog" max-width="460"><v-card rounded="xl"><v-card-title>Create new version</v-card-title><v-card-text><v-text-field v-model="versionName" label="Version name (optional)" variant="outlined" hint="Entries will be copied into a new draft version." persistent-hint /></v-card-text><v-card-actions class="pa-5 pt-0"><v-spacer /><v-btn variant="text" @click="versionDialog = false">Cancel</v-btn><v-btn color="primary" :loading="versioning" :disabled="!selectedAcademicYearId" @click="createVersion">Create Version</v-btn></v-card-actions></v-card></v-dialog>
  </div>
</template>
...
const classLabel = (x) => [x?.grade?.name, x?.section?.display_name || x?.section?.name, x?.stream?.name].filter(Boolean).join(' · ') || 'Class timetable'
...
const loadTimetables = async () => {
  ...
  loading.value = true
  try {
    const data = await timetableApi.weeklyTimetables({ academic_year_id: selectedAcademicYearId.value, per_page: 100 })
    timetables.value = data?.data || data || []

    if (selectedId.value) {
      const selected = timetables.value.find(item => Number(item.id) === Number(selectedId.value))
      if (selected) await selectTimetable(selected)
      else clearWorkspace()
    } else if (timetables.value.length) {
      await selectTimetable(timetables.value[0])
    }
  } catch (e) {
    timetables.value = []
    clearWorkspace()
    ui.showSnackbar(e.response?.data?.message || 'Unable to load timetables.', 'error')
  } finally {
    loading.value = false
  }
}
...
const loadMasters = async () => {
  const results = await Promise.allSettled([
    ...
    api.get('/subjects', { params: { per_page: 999 } }),
    timetableApi.rooms.list({ per_page: 100 }),
    api.get('/bell-schedules/preview'),
  ])
  teachers.value = results[0].status === 'fulfilled' ? extractList(results[0].value) : []
  subjects.value = results[1].status === 'fulfilled' ? extractList(results[1].value) : []
  rooms.value = results[2].status === 'fulfilled' ? (results[2].value?.data || results[2].value || []) : []
  bells.value = results[3].status === 'fulfilled' ? extractList(results[3].value) : []
}

const selectTimetable = async (item) => {
  if (!item?.id || !selectedAcademicYearId.value) return
  selectedId.value = item.id
  gridLoading.value = true
  try {
    const data = await timetableApi.grid(item.id)
    const loadedTimetable = data?.timetable || item

    if (loadedTimetable?.academic_year_id && Number(loadedTimetable.academic_year_id) !== Number(selectedAcademicYearId.value)) {
      clearWorkspace()
      ui.showSnackbar('This timetable does not belong to the selected academic session.', 'warning')
      return
    }

    timetable.value = loadedTimetable
    entries.value = data?.entries || []
    if (!bells.value.length) bells.value = [...new Map(entries.value.map(x => [x.bell?.id, x.bell]).filter(([id]) => id)).values()]
  } catch (e) {
    clearWorkspace()
    ui.showSnackbar(e.response?.data?.message || 'Unable to load timetable grid.', 'error')
  } finally {
...
const lifecycle = async (action) => { if (!confirm(`${action[0].toUpperCase() + action.slice(1)} this timetable?`)) return; try { await timetableApi[action](selectedId.value); await loadTimetables(); ui.showSnackbar(`Timetable ${action}d successfully.`) } catch (e) { ui.showSnackbar(e.response?.data?.message || `Unable to ${action} timetable.`, 'error') } }
const createVersion = async () => { if (!selectedAcademicYearId.value) return; versioning.value = true; try { const created = await timetableApi.createVersion(selectedId.value, versionName.value || null); versionDialog.value = false; versionName.value = ''; await loadTimetables(); if (created?.id) await selectTimetable(created); ui.showSnackbar('New timetable version created.') } catch (e) { ui.showSnackbar(e.response?.data?.message || 'Unable to create version.', 'error') } finally { versioning.value = false } }
const download = async (format) => { if (!selectedId.value || !selectedAcademicYearId.value) return; try { const response = await timetableApi.downloadReport('classes', selectedId.value, format, { academic_year_id: selectedAcademicYearId.value }); const blob = new Blob([response.data], { type: response.headers?.['content-type'] }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `${timetable.value?.name || 'timetable'}.${format === 'excel' ? 'xlsx' : 'pdf'}`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url) } catch (e) { ui.showSnackbar(`Unable to download ${format.toUpperCase()} report.`, 'error') } }

watch(statusFilter, () => { if (selectedId.value && !filteredTimetables.value.some(x => x.id === selectedId.value)) clearWorkspace() })
...
watch(() => appStore.refreshKey, async () => {
  await Promise.all([loadMasters(), loadTimetables()])
})
