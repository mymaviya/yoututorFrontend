  ...
  <v-container fluid>
    <v-card class="rounded-xl">
      <v-card-title class="d-flex align-center justify-space-between">
        <div>
          <div class="text-h6 font-weight-bold">Lesson Import</div>
          <div class="text-caption text-medium-emphasis">
            Import lessons with genre using Excel or CSV.
          </div>
        </div>
      ...
      <v-card-text>
        ...
        <v-file-input
          v-model="file"
          label="Select Lesson Import File"
          accept=".xlsx,.xls,.csv"
          variant="outlined"
...
const downloadTemplate = async () => {
  const res = await api.get('/lessons/import-template', {
    responseType: 'blob',
  })
...
const submitImport = async () => {
  ...
  try {
    ...
    ui.showSnackbar('Lesson import completed successfully', 'success')
  } catch (err) {
