<template>
  <v-container fluid class="status-page pa-0">
    <section class="status-section expired-bg">
        ...
        <v-row justify="center">
          <v-col cols="12" md="7" lg="5">
            <v-card class="status-card animate-item" rounded="xl" elevation="14">
              <v-avatar color="error" size="86" class="mb-6 status-avatar">
              ...
              <p>
                Your free demo or subscription plan has expired. Please renew your
                plan to continue using the ERP.
              </p>

              <div class="status-list">
                <div v-for="item in expiredItems" :key="item">
              ...
              <div class="d-flex justify-center flex-wrap ga-3 mt-7">
                <v-btn color="primary" size="large" rounded="xl" to="/pricing">
                  View Plans
                </v-btn>
...
const expiredItems = [
  'ERP access is currently restricted',
  'Renewal is required to continue',
  'Your data remains safely stored',
  'Contact support for activation help'
]
...
.expired-bg {
  background:
    radial-gradient(circle at top left, rgba(var(--v-theme-error), 0.16), transparent 34%),
...
.hero-glow-one {
  width: 360px;
  height: 360px;
  background: rgba(var(--v-theme-error), 0.36);
  top: -110px;
  left: -100px;
}
...
.status-avatar {
  box-shadow: 0 18px 40px rgba(var(--v-theme-error), 0.28);
}
