<template>
  <v-container fluid class="teacher-timetable-page pa-4 pa-md-6">
        ...
        <v-row>
          <v-col cols="12" md="3">
            ...
            >
              ...
              <v-btn value="class" class="flex-grow-1">
          ...
          <v-col cols="12" sm="6" md="3">
            <v-text-field
              v-model="search"
              label="Search subject, class or room"
    ...
    <v-card rounded="xl" class="timetable-card">
      ...
      <template v-else>
        <div v-if="hasSchedule" class="table-scroll">
          <table class="timetable-grid">
            <thead>
              <tr>
                ...
                <th
                  ...
                  :key="day.value"
                  :class="{ 'today-column': isToday(day.value) }"
                >
            ...
            <tbody>
              <tr v-for="bell in bells" :key="bell.id">
                ...
                <td
                  v-for="day in weekdays"
                  :key="`${bell.id}-${day.value}`"
                  :class="{ 'today-column': isToday(day.value) }"
                >
                  <template v-if="entryFor(day.value, bell.id)" :key="entryFor(day.value, bell.id).id">
                    <div
                      class="period-card"
                      :class="{ 'substitution-card': entryFor(day.value, bell.id).is_substitution }"
                    >
                      <div class="d-flex align-start justify-space-between ga-2">
                        <div class="subject-name">
                          {{ entryFor(day.value, bell.id).subject?.name || 'Subject' }}
                        </div>
                        <v-chip
                          v-if="entryFor(day.value, bell.id).is_substitution"
                          size="x-small"
                          color="warning"
                          variant="flat"
                        >
                      ...
                      <div class="period-meta mt-2">
                        {{ viewMode === 'teacher'
                          ? classLabel(entryFor(day.value, bell.id))
                          : teacherLabel(entryFor(day.value, bell.id)) }}
                      </div>
                      <div v-if="entryFor(day.value, bell.id).room_no" class="period-room mt-1">
                        <v-icon size="14" class="me-1">mdi-door</v-icon>
                        {{ entryFor(day.value, bell.id).room_no }}
                      </div>
                    </div>
                  </template>
        ...
        <div v-else class="empty-state pa-10 pa-md-16 text-center">
          <v-avatar color="primary" variant="tonal" size="78" class="mb-4">
...
const entryIndex = computed(() => {
  ...
  visibleEntries.value.forEach((entry) => {
    const key = `${String(entry.weekday || '').toLowerCase()}-${Number(entry.school_bell_id)}`
    index.set(key, entry)
  })
  return index
})
...
const summaryCards = computed(() => [
  { key: 'weekly_periods', label: 'Weekly Periods', value: summary.value.weekly_periods ?? entries.value.length, icon: 'mdi-calendar-week', color: 'primary' },
  ...
  { key: 'substitutions', label: 'Substitutions', value: summary.value.substitutions ?? entries.value.filter((item) => item.is_substitution).length, icon: 'mdi-account-switch-outline', color: 'warning' },
  { key: 'subjects', label: 'Subjects', value: summary.value.subjects ?? new Set(entries.value.map((item) => item.subject_id).filter(Boolean)).size, icon: 'mdi-book-open-page-variant-outline', color: 'info' },
])
...
const loadTimetable = async () => {
  ...
  try {
    ...
    entries.value = []
    summary.value = {}
    ui.showSnackbar(error?.response?.data?.message || 'Failed to load teacher timetable.', 'error')
  } finally {
...
const exportTimetable = async () => {
  ...
  try {
    ...
    ui.showSnackbar(error?.response?.data?.message || 'Failed to export teacher timetable.', 'error')
  } finally {
...
onMounted(async () => {
  try {
    ...
    ui.showSnackbar(error?.response?.data?.message || 'Failed to load timetable filters.', 'error')
  }
})
</script>
